...
def get_files_protein(files):
    ...
    for a in range(len(carbon)):
        for b in range(len(carbon[a])):
            ...
            axes[b].bar(xplc1[a], numpy.mean(means), color=colors[a], edgecolor='k', yerr=numpy.std(means), error_kw=dict(ecolor='k', lw=3, capsize=5, capthick=3, alpha=0.5), label=labels[a])
            if a > 0:
...
for a in range(len(LC[0])):
    if a == 0:
        for b in range(len(LC[0][a])):
            ax4.bar(x[b], LC[0][a][b], yerr=LC[1][a][b], capsize=2, color=colors[b], width=0.8, edgecolor='k')
            ax5.bar(x[b], PET1[0][a][b], yerr=PET1[1][a][b], capsize=2, color=colors[b], width=0.8, edgecolor='k')
            ax6.bar(x[b], PET2[0][a][b], yerr=PET2[1][a][b], capsize=2, color=colors[b], width=0.8, edgecolor='k')
    else:
        ax4.bar(x, LC[0][a], yerr=LC[1][a], capsize=2, color=colors, width=0.8, edgecolor='k')
        ax5.bar(x, PET1[0][a], yerr=PET1[1][a], capsize=2, color=colors, width=0.8, edgecolor='k')
        ax6.bar(x, PET2[0][a], yerr=PET2[1][a], capsize=2, color=colors, width=0.8, edgecolor='k')
    ttxt_LC, ttxt_PET1, ttxt_PET2 = [], [], []
    for b in range(len(LC[0][a])):
        ttxt_LC.append(LC[0][a][b]+LC[1][a][b]+0.1)
        ttxt_PET1.append(PET1[0][a][b]+PET1[1][a][b]+0.1)
        ttxt_PET2.append(PET2[0][a][b]+PET2[1][a][b]+0.1)
