...
means = [['P', 'TPA', 'BHET', 'PET', 'PETB']]
sd = [['P_sd', 'TPA_sd', 'BHET_sd', 'PET_sd', 'PETB_sd']]
fc = [['P_TPA_fc', 'P_BHET_fc', 'P_PET_fc', 'P_PETB_fc']]
t_test = [['P_TPA_ttest', 'P_BHET_ttest', 'P_PET_ttest', 'P_PETB_ttest']]
t_test_sig = [['P_TPA_ttest_sig', 'P_BHET_ttest_sig', 'P_PET_ttest_sig', 'P_PETB_ttest_sig']]
t_test_TPA = [['TPA_BHET_ttest', 'TPA_PET_ttest', 'TPA_PETB_ttest']]
t_test_TPA_sig = [['TPA_BHET_ttest_sig', 'TPA_PET_ttest_sig', 'TPA_PETB_ttest_sig']]
rel_abun = [['P_relabun', 'TPA_relabun', 'BHET_relabun', 'PET_relabun', 'PETB_relabun']]
fc_TPA = [['TPA_BHET_fc', 'TPA_PET_fc', 'TPA_PETB_fc']]
t_test_BHET = [['BHET_PET_ttest', 'BHET_PETB_ttest']]
t_test_BHET_sig = [['BHET_PET_ttest_sig', 'BHET_PETB_ttest_sig']]
fc_BHET = [['BHET_PET_fc', 'BHET_PETB_fc']]
t_test_PET = [['PET_PETB_ttest']]
t_test_PET_sig = [['PET_PETB_ttest_sig']]
fc_PET = [['PET_PETB_fc']]
...
for a in range(len(rows)):
    if a > 0:
        P, TPA, BHET, PET, PETB = [rows[a][1], rows[a][2], rows[a][3]], [rows[a][4], rows[a][5], rows[a][6]], [rows[a][7], rows[a][8], rows[a][9]], [rows[a][10], rows[a][11], rows[a][12]], [rows[a][13], rows[a][14], rows[a][15]]
        P_mean, TPA_mean, BHET_mean, PET_mean, PETB_mean = numpy.mean(P), numpy.mean(TPA), numpy.mean(BHET), numpy.mean(PET), numpy.mean(PETB)
        this_means = [P_mean, TPA_mean, BHET_mean, PET_mean, PETB_mean]
        this_sd = [numpy.std(P),
                      numpy.std(TPA),
                      numpy.std(BHET),
                      numpy.std(PET),
        ...
        sd.append(this_sd)
        this_fc, this_fc_TPA, this_fc_BHET, this_fc_PET = [], [], [], []
        ...
        this_fc.append((TPA_mean-P_mean)), this_fc.append((BHET_mean-P_mean)), this_fc.append((PET_mean-P_mean)), this_fc.append((PETB_mean-P_mean))
        fc.append(this_fc)
        this_fc_TPA.append((BHET_mean-TPA_mean)), this_fc_TPA.append((PET_mean-TPA_mean)), this_fc_TPA.append((PETB_mean-TPA_mean))
        fc_TPA.append(this_fc_TPA)
        this_fc_BHET.append((PET_mean-BHET_mean)), this_fc_BHET.append((PETB_mean-BHET_mean))
        fc_BHET.append(this_fc_BHET)
        this_fc_PET.append((PETB_mean-PET_mean))
        fc_PET.append(this_fc_PET)
        this_ttest = [stats.ttest_ind(P,TPA)[1], stats.ttest_ind(P,BHET)[1], stats.ttest_ind(P,PET)[1], stats.ttest_ind(P,PETB)[1]]
        t_test.append(this_ttest)
        this_ttest_TPA = [stats.ttest_ind(TPA,BHET)[1], stats.ttest_ind(TPA,PET)[1], stats.ttest_ind(TPA,PETB)[1]]
        t_test_TPA.append(this_ttest_TPA)
        this_ttest_BHET = [stats.ttest_ind(BHET,PET)[1], stats.ttest_ind(BHET,PETB)[1]]
        t_test_BHET.append(this_ttest_BHET)
        this_ttest_PET = [stats.ttest_ind(PET,PETB)[1]]
        t_test_PET.append(this_ttest_PET)
        this_ttest_sig, this_ttest_TPA_sig, this_ttest_BHET_sig, this_ttest_PET_sig = [], [], [], []
        ...
        for d in range(len(this_ttest_BHET)):
            if this_ttest_BHET[d] <= 0.05:
                ...
                this_ttest_BHET_sig.append('')
        for e in range(len(this_ttest_PET)):
            if this_ttest_PET[e] <= 0.05:
                this_ttest_PET_sig.append('+')
            else:
                this_ttest_PET_sig.append('')
        t_test_sig.append(this_ttest_sig)
        t_test_TPA_sig.append(this_ttest_TPA_sig)
        t_test_BHET_sig.append(this_ttest_BHET_sig)
        t_test_PET_sig.append(this_ttest_PET_sig)
...
with open(new_fn, 'w') as f:
    writer = csv.writer(f)
    for a in range(len(rows)):
        writer.writerow(rows[a]+rel_abun[a]+means[a]+sd[a]+fc[a]+fc_TPA[a]+fc_BHET[a]+fc_PET[a]+t_test[a]+t_test_sig[a]+t_test_TPA[a]+t_test_TPA_sig[a]+t_test_BHET[a]+t_test_BHET_sig[a]+t_test_PET[a]+t_test_PET_sig[a])
"""
