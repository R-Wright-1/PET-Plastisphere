...
raw_data = pd.read_csv('DT_PET.csv', index_col=0, header=0)
...
for ax in axis:
    ax.plot(list(raw_data_mean.index.values), raw_data_mean.loc[:, 'Control'], color=colors[0], label=labels[0])
    ax.plot(list(raw_data_mean.index.values), raw_data_mean.loc[:, 'Thioclava'], color=colors[1], label=labels[1])
    ax.plot(list(raw_data_mean.index.values), raw_data_mean.loc[:, 'Bacillus'], color=colors[2], label=labels[2])
    ax.plot(list(raw_data_mean.index.values), raw_data_mean.loc[:, 'Community'], color=colors[3], label=labels[3])
...
for a in range(len(wl)):
    xplt.append(x[2])
    a = wl[a]
    ref = pd.DataFrame(raw_data.loc[1410, :]).transpose()
    line = pd.DataFrame(raw_data.loc[a, :]).transpose()
    line = line.divide(ref.values[0])
    m1, m2, m3, m4 = numpy.mean(line.loc[:, 'Control'].values[0]), numpy.mean(line.loc[:, 'Thioclava'].values[0]), numpy.mean(line.loc[:, 'Bacillus'].values[0]), numpy.mean(line.loc[:, 'Community'].values[0])
    s1, s2, s3, s4 = numpy.std(line.loc[:, 'Control'].values[0]), numpy.std(line.loc[:, 'Thioclava'].values[0]), numpy.std(line.loc[:, 'Bacillus'].values[0]), numpy.std(line.loc[:, 'Community'].values[0])
    ax6.bar(x, [m1, m2, m3, m4], yerr=[s1, s2, s3, s4], color=colors, edgecolor='k', width=0.8, error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
    x = [b+5 for b in x]
plt.sca(ax6)
